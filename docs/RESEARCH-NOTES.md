...
| **Speculative positioning (COT).** Speculators' net futures positions move WITH exchange rates far more reliably than they predict them. | Klitgaard & Weir (2004), *Exchange rate changes and net positions of speculators in the futures market*, FRBNY Economic Policy Review 10(1) | COT is never a direction signal |
| **Crowded positions crash.** Speculators' net positions predict currency crash risk (negative skewness). | Brunnermeier, Nagel & Pedersen (2008), as above | The `cot_crowding` layer shrinks trades on the side of a multi-year extreme (`sentinel/data/cot.py`) |
| **The dollar factor.** A dollar-wide risk factor explains much of the common variation in currency returns. | Lustig, Roussanov & Verdelhan (2014), *Countercyclical currency risk premia*, JFE 111(3) 527–553 | The rebuilt DXY (`sentinel/data/dxy.py`), its features on every signal, and the `dxy_headwind` layer |
