...
if [ "$QUICK" -eq 0 ]; then
    ...
    probe "CFTC COT data (positioning)"     https://publicreporting.cftc.gov/ 0
    probe "OANDA practice API"              https://api-fxpractice.oanda.com/ 0
    hint "an unreachable news, AI, TradingView, Telegram, Bale or CFTC endpoint only disables that feature; trading is unaffected"
